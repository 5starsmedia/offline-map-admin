...
class NewsCategoriesCtrl {
  /*@ngInject*/
  constructor($scope, $state, $filter, $stateParams, NewsRegionModel, postType) {
    ...
    var loadData = () => {
      NewsRegionModel.getTree({ postType: postType }, function (res) {
        $scope.loading = false;
        $scope.tree = res;
      });
    };
    ...
    $scope.addItem = function (child) {
      ...
      child.$insertItem(function (item) {
        ...
        item.focus = true;
        item.$settings = true;
        $state.go(postType + '.regions.edit', { id: item._id });
      });
    };
